        ...
        <div class="bg-white rounded-xl shadow-md p-6 mb-8">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-600 mt-2">Buat transaksi peminjaman buku baru</p>
                </div>
            ...
            <form action="{{ route('admin.transaksi.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="buku_id" id="buku_id"
                        ...
                        <option value="{{ $item->id }}">{{ $item->judul }} - {{ $item->penulis }} (Stok: {{ $item->stok }})</option>
                        @endforeach
                    </select>
                    @error('buku_id')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <p class="text-xs text-blue-600 mt-2">
                        Tanggal jatuh tempo akan dihitung otomatis berdasarkan tanggal pinjam + lama peminjaman.
                    </p>
                </div>
                @endif
